...
def generateDependencyDict(
    heuNets, objectTypes, ev, inconsumableObjects=[], inconsumableThreshold=1
):
...
def getClosestPredecessor(
    log, position, activity, activityList, objectType, predecessors
):
    if position == 0:
        return None, None
    else:
...
def generateClosestPredecessorDF(objectSet, eventLogDict, predecessors):
    ...
    for objectID in objectSet:
        ...
        for i in range(len(uniqueLogSnippet)):
            ...
            closestPredecessor, index = getClosestPredecessor(
                uniqueLogSnippet,
                i,
                activity,
                activityList,
...
def generateClosestSuccessorDF(objectSet, eventLogDict, successors):
    ...
    for objectID in objectSet:
        ...
        for i in range(len(uniqueLogSnippet)):
            ...
            objectType = uniqueLogSnippet["object_type"].iloc[0]
            event_id = uniqueLogSnippet["event_id"].iloc[i]
            closestSuccessor, index = getClosestSuccessor(
                uniqueLogSnippet,
                i,
                activity,
...
def mineOutputBindings(
    ...
    closestPredecessorDict = {
        k: v for k, v in closestPredecessorDF.groupby("predecessor_event_id")
    }
    defaultClosestPredecessorDict = pd.DataFrame(
        data=None, columns=closestPredecessorDF.columns
    )
    eventLogForMinerDict = {k: v for k, v in eventLogForMiner.groupby("event_id")}
    defaultEventLogForMinerDict = pd.DataFrame(
        data=None, columns=eventLogForMiner.columns
    )
    for event in eventIDSet:
        print(
            "event " + str(event) + " , " + str(eventCounter) + " of " + str(eventLen)
        )
        ...
        # return snippet
        snippetObjectTypeDict = {
            k: v["event_id"] for k, v in snippet.groupby("object_type")
        }
        defaultSnippetObjectTypeDict = pd.DataFrame(data=None, columns=snippet.columns)[
            "event_id"
        ]
        snippetObjectTypeEventIDDict = {
            k: v["object"] for k, v in snippet.groupby(["object_type", "event_id"])
        }
        defaultSnippetObjectTypeEventIDDict = pd.DataFrame(
            data=None, columns=snippet.columns
        )["object"]
        ...
        start_time = time.time()
        allEventObjects = eventLogForMinerDict.get(event, defaultEventLogForMinerDict)[
            ["object_type", "object"]
        ].values
        ...
        for objType in allObjectTypes:
            ...
            start_time = time.time()
            succSet = set(
                snippetObjectTypeDict.get(objType, defaultSnippetObjectTypeDict)
            )
            ...
            for succ in succSet:
                ...
                start_time = time.time()
                objSet = frozenset(
                    snippetObjectTypeEventIDDict.get(
                        (objType, succ), defaultSnippetObjectTypeEventIDDict
                    )
                )
            ...
            for k in range(len(seq) + 1):
                ...
                if totalCombinations > combo_threshold:
                    print(
                        "Too many possibilities for outputbindings. Skipped event "
                        + str(event)
                    )
    ...
    for binding in outputBindingsSingleCounts:
        ...
        if key in outputBindingsRanges.keys():
            outputBindingsRanges[key].append(
                tuple({(act, objType): count} for act, objType, count, _ in binding[1])
            )
        else:
            outputBindingsRanges[key] = [
                tuple({(act, objType): count} for act, objType, count, _ in binding[1])
            ]
...
def mineInputBindings(
    ...
    eventCounter = 1
    eventLen = len(eventIDSet)
    closestSuccessorDict = {
        k: v for k, v in closestSuccessorDF.groupby("successor_event_id")
    }
    defaultClosestSuccessorDict = pd.DataFrame(
        data=None, columns=closestSuccessorDF.columns
    )
    eventLogForMinerDict = {k: v for k, v in eventLogForMiner.groupby("event_id")}
    defaultEventLogForMinerDict = pd.DataFrame(
        data=None, columns=eventLogForMiner.columns
    ...
    for event in eventIDSet:
        ...
        snippetObjectTypeDict = {
            k: v["event_activity"] for k, v in snippet.groupby("object_type")
        }
        defaultSnippetObjectTypeDict = pd.DataFrame(data=None, columns=snippet.columns)[
            "event_id"
        ]
        snippetObjectTypeEventIDDict = {
            k: v["object"]
            for k, v in snippet.groupby(["object_type", "event_activity"])
        }
        defaultSnippetObjectTypeEventIDDict = pd.DataFrame(
            data=None, columns=snippet.columns
        )["object"]
        ...
        for objType in allObjectTypes:
            ...
            for pred in predSet:
                ...
                objSet = frozenset(
                    snippetObjectTypeEventIDDict.get(
                        (objType, pred), defaultSnippetObjectTypeEventIDDict
                    )
                )
                for obj in objSet:
                    try:
    ...
    for binding in inputBindingsSingleCounts:
        ...
        if key in inputBindingsRanges.keys():
            inputBindingsRanges[key].append(
                tuple({(act, objType): count} for act, objType, count, _ in binding[1])
            )
        else:
            inputBindingsRanges[key] = [
                tuple({(act, objType): count} for act, objType, count, _ in binding[1])
            ]
...
def filter4(inputBindings, outputBindings, threshold, activityCount):
    ...
    def filterByTreshold(bindings, activity):
        filteredBindings = [
            x for x in bindings if x[1] / activityCount[activity] > threshold
        ]
        return filteredBindings

    def getMostFrequent(bindings):
        mostFrequentBinding = bindings[
            [x[1] for x in bindings].index(max([x[1] for x in bindings]))
        ]
        ...
        for obligation in mostFrequentBinding[0]:
            if obligation[0] in inputBindings.keys():
                possibleInputBindings = [
                    x
                    for x in inputBindings[obligation[0]]
                    if (activity, obligation[1]) in [(y[0], y[1]) for y in x[0]]
                ]
        ...
        for obligation in mostFrequentBinding[0]:
            if obligation[0] in outputBindings.keys():
                possibleOutputBindings = [
                    x
                    for x in outputBindings[obligation[0]]
                    if (activity, obligation[1]) in [(y[0], y[1]) for y in x[0]]
                ]
...
def visualizer(
    ...
    objectColors = {
        list(occnet.objectTypes)[i]: colorPalette[i]
        for i in range(len(occnet.objectTypes))
    }
    ...
    for activity, bindings in occnet.inputBindings.items():
        ...
        for binding in bindings:
            inputObligationPositions[activity][
                tuple((tuple(binding[0]), binding[1]))
            ] = {}
            for obligation in binding[0]:
                ...
                if column in inputObligationColsToRowsDict[activity].keys():
                    inputObligationColsToRowsDict[activity][column].append(
                        rowDict[obligation[0]][obligation[1]]
                    )
                else:
                    inputObligationColsToRowsDict[activity][column] = [
                        rowDict[obligation[0]][obligation[1]]
                    ]
                if (
                    rowDict[obligation[0]][obligation[1]]
                    in inputObligationRowsToColsDict[activity].keys()
                ):
                    inputObligationRowsToColsDict[activity][
                        rowDict[obligation[0]][obligation[1]]
                    ].append(column)
                ...
                if obligation[0] in inputObligationEdgeDict[activity].keys():
                    if (
                        obligation[1]
                        in inputObligationEdgeDict[activity][obligation[0]].keys()
                    ):
                        inputObligationEdgeDict[activity][obligation[0]][
                            obligation[1]
                        ].append((column, row))
                    else:
                        inputObligationEdgeDict[activity][obligation[0]][
        ...
        for binding in bindings:
            ...
            for obligation in binding[0]:
                ...
                if column in outputObligationColsToRowsDict[activity].keys():
                    outputObligationColsToRowsDict[activity][column].append(
                        rowDict[obligation[0]][obligation[1]]
                    )
                else:
                    outputObligationColsToRowsDict[activity][column] = [
                        rowDict[obligation[0]][obligation[1]]
                    ]
                if (
                    rowDict[obligation[0]][obligation[1]]
                    in outputObligationRowsToColsDict[activity].keys()
                ):
                    outputObligationRowsToColsDict[activity][
                        rowDict[obligation[0]][obligation[1]]
                    ].append(column)
                ...
                if obligation[0] in outputObligationEdgeDict[activity].keys():
                    if (
                        obligation[1]
                        in outputObligationEdgeDict[activity][obligation[0]].keys()
                    ):
                        outputObligationEdgeDict[activity][obligation[0]][
                            obligation[1]
                        ].append((column, row))
                    else:
                        outputObligationEdgeDict[activity][obligation[0]][
    ...
    for activity in dependencyGraph._node.keys():
        ...
        parentNodes.append(
            ParentNode(
                activity,
                occnet.dependencyGraph,
                occnet.inputBindings,
                occnet.outputBindings,
                dimensions,
                activityPosition[activity],
                inputMax,
                outputMax,
            )
        )
        activityNodes.append(
            ActivityNode(
                activity,
                objectColors,
                occnet.inputBindings,
        ...
        if activity in occnet.inputBindings.keys():
            for binding in occnet.inputBindings[activity]:
                for obligation in binding[0]:
                    row = inputObligationPositions[activity][
                        tuple((tuple(binding[0]), binding[1]))
                    ][obligation]["row"]
                    column = inputObligationPositions[activity][
                        tuple((tuple(binding[0]), binding[1]))
                    ][obligation]["column"]
                    inputNodes.append(
                        InputNode(
        ...
        if activity in occnet.outputBindings.keys():
            for binding in occnet.outputBindings[activity]:
                for obligation in binding[0]:
                    row = outputObligationPositions[activity][
                        tuple((tuple(binding[0]), binding[1]))
                    ][obligation]["row"]
                    column = outputObligationPositions[activity][
                        tuple((tuple(binding[0]), binding[1]))
                    ][obligation]["column"]
                    outputNodes.append(
                        OutputNode(
    ...
    for act1 in outestOutputDict.keys():
        for act2 in outestOutputDict[act1].keys():
            for objectType in outestOutputDict[act1][act2].keys():
                activityEdges.append(
                    ActivityEdge(
                        "output_"
                        + act1.replace(" ", "_")
                        + "_"
                        + str(outestOutputDict[act1][act2][objectType][1])
                        + "_"
                        + str(outestOutputDict[act1][act2][objectType][0]),
                        "input_"
                        + act2.replace(" ", "_")
                        + "_"
                        + str(outestInputDict[act2][act1][objectType][1])
                        + "_"
                        + str(outestInputDict[act2][act1][objectType][0]),
                        act1,
                        act2,
                    )
                )
...
class SimpleOCCNet:
    def __init__(
        ...
        self.inputBindings, self.outputBindings = filter4(
            inputBindings,
            outputBindings,
            self.relativeOccuranceThreshold,
            activityCount,
        )
        self.objectTypes = {
            objectType
    ...
    def to_OCCausalNet(self) -> OCCausalNet:
        """
        ...
        def _convet_marker_group(marker_group):
            ...
            for marker in marker_group[0]:
                markers.append(
                    OCCausalNet.Marker(marker[0], marker[1], (marker[3][0], marker[3][1]), marker[2])
                )
...
class ParentNode:
    def __init__(
        ...
        self.height += (
            max((0, max((inputObligationMax, outputObligationMax)) - 2))
            * dimensions["additional_h"]
        )
        self.pos_y -= (
            max((0, max((inputObligationMax, outputObligationMax)) - 2))
            * dimensions["additional_h"]
...
class ActivityNode:
    def __init__(
        self, activity, objectColors, inputBindings, outputBindings, parentNode
    ):
        self.activity = activity
        ...
        self.pos_x = (
            (parentNode.width - self.width) / 2
            + len(self.inputBindings) * dimensions["additional_w"] / 2
            - len(self.outputBindings) * dimensions["additional_w"] / 2
        )
...
class InputNode:
    def __init__(
        ...
        self.activity = activity
        self.id = (
            "input_" + activity.replace(" ", "_") + "_" + str(row) + "_" + str(column)
        )
        self.index = index
...
class OutputNode:
    def __init__(
        ...
        self.activity = activity
        self.id = (
            "output_" + activity.replace(" ", "_") + "_" + str(row) + "_" + str(column)
        )
        self.index = index
        self.objectType = objectType.replace(" ", "_")
        self.width = dimensions["obligationNode_w"]
...
# %%
def discover_occn_fhm(eventLog, eventLogForMiner, objectTypes, relativeOccuranceThreshold, outputDirName=None, visualize=False) -> OCCausalNet:
    """
    Discover an OCCN using the Flexible Heuristics Miner (FHM) algorithm.
    
    Parameters:
    - eventLog: The event log containing the events.
    - eventLogForMiner: The event log to be used for mining.
    - objectTypes: A list of object types to be considered in the mining process.
    - relativeOccuranceThreshold: The threshold for relative occurrence of markers (between 0 and 1).
    - outputDirName: The name of the directory where the results will be dumped. Leave None for no dumping.
    - visualize: Boolean indicating whether to visualize the discovered OCCN.
    
    Returns:
    - occn: An OCCausalNet object representing the discovered object-centric causal net.
    """
    dependency_threshold = 0.5 
    and_threshold = 0.65
    loop_two_threshold = 0.5 
    ...
    startActivities, endActivities, dependencyGraph, outputBindings, outputSkippedEvents, inputBindings, inputSkippedEvents = _discover(eventLog, eventLogForMiner, objectTypes, dependency_threshold, and_threshold, loop_two_threshold, combo_threshold)
    ...
    if outputDirName:
        _dump_occn(outputDirName, activityCount, dependency_threshold, loop_two_threshold, combo_threshold, dependencyGraph, outputBindings, outputSkippedEvents, inputBindings, inputSkippedEvents)
    if visualize:
        visualizer(
            occnet,
            dependencyGraph,
...
def occn_from_dump(inputDirName, relativeOccuranceThreshold, visualize=False) -> OCCausalNet:
    """
    Load an OCCN from a dump directory.
    
    Parameters:
    - inputDirName: The name of the directory where the OCCN was dumped. Must be stored in "evaluation/discovered_occns/".
    - relativeOccuranceThreshold: The relative occurrence threshold to be used.
    - visualize: Boolean indicating whether to visualize the loaded OCCN.

    Returns:
    - occn: An OCCausalNet object representing the loaded object-centric causal net.
    """
    inputPath = "evaluation/discovered_occns/" + inputDirName + "/"
    
    with open(inputPath + "dependencyGraph.pkl", "rb") as f:
        dependencyGraph = pickle.load(f)
    with open(inputPath + "outputMarkers.pkl", "rb") as f:
        outputBindings = pickle.load(f)
    with open(inputPath + "inputMarkers.pkl", "rb") as f:
        inputBindings = pickle.load(f)
    with open(inputPath + "activityCount.pkl", "rb") as f:
...
def _discover(eventLog, eventLogForMiner, objectTypes, dependency_threshold, and_threshold, loop_two_threshold, combo_threshold):
    ...
    dependencyDict, startActivities, endActivities = generateDependencyDict(
        heuNets, objectTypes, events, [], 1
    )
    ...
    closestPredecessorDF = generateClosestPredecessorDF(
    ...
    outputBindings, times, outputSkippedEvents = mineOutputBindings(
        eventIDSet,
        eventToActivityDict,
        closestPredecessorDF,
        events,
    ...
    inputBindings, inputSkippedEvents = mineInputBindings(
        eventIDSet,
        eventToActivityDict,
        closestSuccessorDF,
        events,
    ...
    return startActivities,endActivities,dependencyGraph,outputBindings,outputSkippedEvents,inputBindings,inputSkippedEvents

def _dump_occn(outputDirName, activityCount, dependency_threshold, loop_two_threshold, combo_threshold, dependencyGraph, outputBindings, outputSkippedEvents, inputBindings, inputSkippedEvents):
    outputPath = "evaluation/discovered_occns/" + outputDirName + time.strftime("_%Y-%m-%d-%H-%M-%S/", time.gmtime())
